...
async function inspectPublication(env:Bindings,userId:string,input:unknown) {
 ...
 let projected:ReturnType<typeof communityProjection>;try{projected=communityProjection(documentSchema.parse(JSON.parse((await decodeDocument(env,row!.document))??'')));}catch(error){fail(400,'unsafe_public_projection',error instanceof Error?error.message:'The public projection could not be validated.');}const document=projected!.document;
