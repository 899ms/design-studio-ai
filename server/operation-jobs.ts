...
async function ownedJob(c:Context<Env>){await projectRow(c,c.req.param('id')!, { document: false });const row=await c.env.DB.prepare('SELECT * FROM operation_jobs WHERE project_id=? AND user_id=? AND operation_id=?').bind(c.req.param('id'),owner(c),c.req.param('operationId')).first<JobRow>();if(!row)fail(404,'not_found','Unknown operation');return row!;}
...
operationRoutes.post('/:id/operations',async c=>{
 const row=await projectRow(c,c.req.param('id')!,{document:false}),body=operationJobSchema.parse(await c.req.json()),encoded=JSON.stringify(body),hash=await paintHash(new TextEncoder().encode(encoded));
