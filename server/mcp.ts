...
export async function handleMcp(c: Context<Env>, app: Hono<Env>) {
  ...
  server.registerTool('author_scene',{description:'Preview or apply a shared 3D command: mesh, rig, clip, material, terrain, and page-level camera, camera-key, environment (lights, fog, rendering, sky/panorama, post effects), emitter (presets snowfall, snow-burst, frost-breath, ground-mist), remove-emitter and light (add, patch or remove one extra light by id). Mesh actions include subdivide, unsubdivide (undo subdivide steps to cut density), sculpt (smooth, inflate, move, crease, flatten, pinch; optional path and symmetry) and remove-node. Page-level actions merge into the scene instead of replacing it. Pass an array of up to 64 commands to apply them in order as one saved revision; if any command fails, nothing is saved. Requires current revision; preview defaults to true. responseMode "summary" returns changed IDs and compact node summaries.',inputSchema:{projectId:z.string(),...sceneRequestSchema.shape}},async ({projectId,...body})=>callApi('POST',`/api/projects/${encodeURIComponent(projectId)}/scene`,body));
