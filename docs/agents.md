...
Inspection renders 3D scenes at the tile size, not the page size, and each capture has a 45-second deadline. A missed deadline returns `504 render_timeout` and no image. To fix it, lower `maxDimension`, or remove transmission from most materials and reduce depth of field, light shafts and emitter density. Retrying the same request will not help.
