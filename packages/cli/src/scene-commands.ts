...
export function registerSceneCommands(program:Command,client:()=>Client){
  ...
  group.command('inspect <projectId>').option('--page <id>','Page ID').option('--time <seconds>','Sample pose time','0').option('--summary','Counts, bounds and issues without bone positions or topology diagnostics').action(async(projectId,options)=>{const time=z.coerce.number().finite().min(0).max(3600).parse(options.time);output(await client().json(`/api/projects/${encodeURIComponent(projectId)}/scene?${new URLSearchParams({time:String(time),...(options.page?{pageId:options.page}:{}),...(options.summary?{detail:'summary'}:{})})}`));});
  group.command('command <projectId>').requiredOption('--page <id>','Page ID').requiredOption('--revision <number>','Observed project revision').requiredOption('--file <path>','Command JSON or - for stdin').option('--apply','Save the command; default is read-only preview').option('--summary','Return changed IDs and compact node summaries').action(async(projectId,options)=>{const body=sceneRequestSchema.parse({pageId:options.page,expectedRevision:Number(options.revision),command:await inputJson(options.file),preview:!options.apply,responseMode:options.summary?'summary':'full'});output(await client().json(`/api/projects/${encodeURIComponent(projectId)}/scene`,'POST',body));});
}
