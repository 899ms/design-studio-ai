...
export const sceneCommandSchema = z.discriminatedUnion('action', [
  ...
  z.object({action:z.literal('sculpt'),nodeId:id,center:vectorSchema,radius:finite.min(.0001).max(1000),strength:finite.min(0).max(1).default(.2),mode:z.enum(['smooth','inflate','move','crease','flatten','pinch']),delta:vectorSchema.default([0,0,0]),path:z.array(vectorSchema).max(64).default([]),symmetry:z.boolean().default(false)}),
